...
A qualitative guess would suggest that abundance-weighted Z\textsubscript{C} of IPLs is highest in the oxic zone of the Black Sea water column, and gradually decreasing, or becoming more reduced, with depth and anoxia. Oxia/anoxia is only one set of variables changing with depth in the Black Sea to which organisms must adapt, and there are certainly others (pressure, salinity, light availablity, \textit{etc.}), but membrane adaptation to the system as a whole has manifested in parallels with those found in hydrothermal gradients, undoubtedly because certain alkyl chain modifications function in both systems. The question then becomes; what drove natural selection to converge upon similar distributions of alkyl chain structures in bs and in hot sp? What benefit is there to having red red ox ox?

thermo studies of prot



The same lipid modifications providing membrane thermostability along thermal redox gradients are also present providing stability along isothermal redox gradients, undoubtedly because these modifications serve their function adequately in both systems and potentially because synthesizing oxidized lipids is more cost effective in oxic conditions, and the same for reduced lipids in reducing conditions.





Adaptations to provide stable membranes at high temperatures manifested primarily as reduced chemical modifications in IPL alkyl chains. 

spatially along these thermal and redox gradients 


% more reduced, more oxidized conditions - more reduced, more oxidized lipid adaptations. Not only for lipids, but for protein adaptation to thermo (ZC). Subsequently showed that not only were proteins thermostable, but energetically favorable. Apply a more in-depth thermodynamic analysis of lipids to predict distributions. Is lipid ZC a proxy for cost-effective thermo adaptation along redox gradient?

% future work: different kinds of lipid adaptations that, to the best of our knowledge, have similar effects on the biophysical properties of lipid membranes:

% Sterols and hopanes have been shown to condense membranes and decrease permeability - these have hydrocarbon core structures with lots of reduced carbon with a low ZC. Compare to expression of trans-unsaturated fatty acids, which are also thought to condense membranes and decrease permeability, but the double bond is oxidized. Also compare to GDGT rings, which have been proposed to have a condensing/packing effect while increasing fluidity. Compare to H-branching in GDGTs, which is more oxidized than an analogous non-branched GDGT, which could be an oxidized adaptation to decrease fluidity by increasing packing (check this). Polar headgroups contributing to rigidity (refs in Daniel and Cowan 2000).

% Compare cis-unsaturated fatty acids, which increase membrane fluidity by decreasing packing, to anteiso methylated fatty acids which are thought to do the same; the former is oxidized, the latter is reduced. Also compare to cyclopropane rings in alkyl chains, which seems like another reduced way to perform a similar function as a cis-unsaturation.

% Daniel and Cowan 2000 ... "With the publication of lipid analyses of an increasingly diverse range of hyperthermophilic archaea, the diversity of lipids is rapidly increasing with, for example, the identification of fatty acids in the marine archaeon P.furiosus, unsaturated diether lipids in Methanopyrus kandleri [116], 36-membered macrocyclic diether lipids in Methanococcus jannaschii (fig. 3g) [117] and tetraether lipids containing a covalent cross-link at the centre of the isoprenoid chains in Methanothermus fervidus (fig. 3h) [118]. There is currently no suggestion that any of these more exotic structures have a specific role in thermophily". If these lipids have no role in thermophily, perhaps their distributions can be better explained by adaptation to redox?

% Daniel and Cowan 2000 ... "It has been suggested [119] that interglycosyl headgroup hydrogen-bonding interactions further stabilise the membrane structure, possibly by reducing lateral lipid mobility" headgroups another way to regulate biophysical properties



% incorporation of 1,2-alkanediols?

% if time, create a table of ideas for oxidized and reduced ways of influencing membrane biophysical properties.

% if time, implications of ZC trends on interpreting lipid ring indices
