...
Attempts to tie a particular lipid trait to a specific organism or geochemical variable have proven troublesome. The discovery of 2-methylhopanoids in an anoxygenic phototroph \citep{rashby2007biosynthesis} required reinterpretation of these lipids as a biomarker for cyanobacteria \citep{summons1992methylhopanoids}, since then 2-methylhopanoids can be linked to a variety of bacteria \citep{ricci2014diverse}. The lipid crenarchaeol, previously thought to be a biomarker for planktonic marine archaea and a proxy for marine temperature, was later found in Nevada hot springs between 40 and 84$^{\circ}$C with no correlation between temperature and distribution \citep{pearson2004nonmarine}. Thermophilic archaea were once the only known source of GDGT lipids until the discovery that GDGTs are widespread in environments below 20$^{\circ}$C \cite{schouten2000widespread} and that non-isoprenoidal GDGTs can be synthesized by anaerobic bacteria \citep{weijers2006membrane}. The number of rings in GDGT alkyl chains is often used in `ring indices' as a proxy for temperature \citep{schouten2002distributional, tierney2012gdgt} though conflicting reports have led to questions about the temperature-dependence of GDGT ring expression entirely \citep{sollich2017heat}. Efforts to assign lipids as biomarkers or proxies for a single organism or environmental variable are often met with surprising exceptions because many structural adaptations in lipids seemingly function in a wide variety of natural systems and extremes.

Take, for instance, the intriguing parallels between spatial distributions of lipids sampled across two natural gradients; the first relatively isothermal and the second spanning a wide temperature range.

across the relatively isothermal redox gradient of Black Sea water columns and sediment (refs... flo's 2009 paper, schrodinger's thesis), and among thermophilic microorganisms thriving along the temperature and water chemistry gradients of a hydrothermal system. When sampling from the oxic to anoxic zones of a Black Sea water column, or from the cooler, downstream area of a terrestrial hydrothermal system toward the hot spring source, similar patterns tend to emerge in lipid structure, with shorter-length, relatively unsaturated, bilayer-forming, ester-linked alkyl chain lipids abundant at one end and longer-chain, saturated, monolayer-forming, ether-bonded lipids abundant at the other.

For communities of thermophilic microorganisms, lipid composition changes spatially along thermal gradients, which in the case of hydrothermal systems, also tends to coincide with gradients in pH, solute concentrations, and oxidation-reduction (redox) potential.

Ch2 energy stuff:
Ongoing efforts are needed to elucidate the factors that ultimately control lipid distribution in the environment. Lipids obviously need to function for an organism and to have an acceptable biosynthetic cost. Evolution will tend to select for lipid structures that confer fitness advantage according to this cost/benefit analysis. We have a general understanding of how certain lipid adaptations are supposed to work (e.g. chain length and cis double bonds in regulating membrane fluidity and permeability, ether bonds in resisting acid hydrolysis, etc.), though we are still discovering how various lipid structural characteristics function (e.g. isoprenoidal methylation, GDGT rings) and the function of some structures are almost completely unknown (BHP side chains).
