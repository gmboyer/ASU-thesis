...
% Relative abundances of BHP headgroup functionality (refer to bar plot: dominated by tetrol and pentol in most sites, some aminotriol)
...
As shown in in the left panel of Figure \ref{fig:aminopolyol_predict}, the fraction of aminoBHPs in observed in lipid extracts is approximately 1-2 orders of magnitude greater than what is predicted by thermodynamic predictions of metastable equilibrium. This could be due to the sources of error in the way the thermodynamic properties of BHP headgroups were estimated, an unrepresentative choice of basis species, uncertainties in the underlying experimental data, \textit{etc}. It is possible that aminopolyol headgroup distributions in nature are responding to variables that are not captured in this thermodynamic model or its underlying energetic assumptions. If one assumes the reverse is true, the discrepancy between predicted and observed fractions could be explained potential differences in ionization efficiency between BHPs with aminopolyol and polyol headgroups during LC-MS analysis. Due to a lack of commercially-available standards, response factor correction could not be performed during the quantification of BHP structures. For the fraction of predicted aminopolyol headgroups to approach observation, BHP LC-MS integrated peak areas would need a response factor correction of about 0.04x, or 25x for non-aminopolyols (Figure \ref{fig:aminopolyol_predict}, right panel). \cite{cooke2008bacterial} and \cite{van2012bacteriohopanepolyol} reported that among a suite of five BHP authentic standards, peak areas of BHPs with nitrogen were approximately 12/8 as intense as those without, meaning a response correction of about 8/12x, or 0.7x, would need to be applied to nitrogen-bearing BHP peak areas to help ensure consistency with BHPs without nitrogen.

% Fraction of aminoBHPs are not predicted very well. Could also very well be due to the way aminoBHP properties are estimated, or even that aminoBHP abundance is not predictable by the thermodynamic assumptions of this study. Could also be due to a lack of commercially-available authentic BHP standards for aminopolyol vs. polyol quantification correction. Would require 0.04x RF for aminopolyols (or assuming 25x RF for non-aminopolyols) to get them on the predicted line. Not incredibly out of the question, as BHPs with nitrogen have been reported to have different ionization efficiency relative to those without (Talbot or Welander?), though these response factors were obtained on a different set of instrumentation than this study, and that is known to be an issue (affected by flow pressure, method, instrument, etc. etc.). Furthermore, a study of IPLs found that nitrogen-bearing IPLs were typically more intense than those without (may be same for this study, leading to overestimation of aminoBHPs). However, while the LC-MS/MS model was the same, the method and ionization source were different for IPLs than for BHPs. Requires further study. 
