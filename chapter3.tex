\chapter{Thermodynamic assessment of redox controls on bacteriohopanepolyol headgroups}
...
\section{Methods}
\paragraph*{Analysis of water samples} Water temperature and pH were measured immediately prior to collection of water and biological samples. Water samples were filtered with ...[need water sample prep, spectro, IC, DIC, and ICPMS methods]... Oxidation-reduction potentials for select half-reactions were calculated using EQ3/6 geochemistry software using data from the slop16 aqueous thermodynamic database. Output files and the slop16 database are given in the supplementary material.

\paragraph*{HPLC-MS} Total lipid extracts (TLEs) from Yellowstone hot spring sediments and biofilms were obtained for this sample set as reported in \cite{boyer2018thermophile}.
...
Steps one, two, and three of the workflow shown in Figure \ref{fig:BHP_thermoest_paper} involve the addition and subtraction of second order group contributions to thermodynamic properties given in Table \ref{tab:grpadd} to modify the properties of sugar alcohols (structure A) into those estimated for non-amino BHP headgroups (structure D). In step 1, the properties of structure B are obtained from structure A by subtracting the contribution from C-(H)\textsubscript{2}(O)(C), the properties of a CH\textsubscript{2} group bonded to one carbon and one oxygen, and adding the contribution from C-(H)(O)(C)\textsubscript{2}, the properties of a CH group bonded to two carbons and one oxygen. In step 2, the contribution of C-(H)\textsubscript{2}(C)\textsubscript{2}, a CH\textsubscript{2} group bonded to two carbons, is added to structure B a number of times such that structure C has six carbons. Step 2 is skipped for hexafunctional structure B, as it already has six carbons. In step 3, contributions from C-(H)(C)\textsubscript{3} and C-(H)\textsubscript{3}(C) are then added to structure C (or structure B for the hexafunctional workflow), resulting in estimated properties for six-carbon-long BHP tetrol, pentol, and hexol headgroups (structure D). In step 4, the properties of 1,2-ethanediol (given in Table \ref{tab:sug_alc}) are subtracted from structure D and those of ethanolamine (given in Table \ref{tab:misc_prop}) are added, resulting in the estimated properties of BHP aminotriol, aminotetrol, and aminopentol headgroups (structure E). In step 5, the properties of ionized aminopolyol BHP headgroups (structure F) are obtained by subtracting the properties of ethanolamine and adding those of ethanolamminium, given in \ref{tab:misc_prop}.


% Add butantetrol, pentanepentol, and hexanehexol to table.
% Pull up BHP_thermo.xlsx and get refs for CH2O, CHO, and CH2 aqueous props to finalize discussion of polyol headgroup estimation.
% Get refs for MEA and protonation rxn used in estimation of aminopolyols.
% Table could become "Partial molal thermodynamic data for sugar alcohols and other components used in the estimation of BHP polar headgroup properties".


% Group additivity theory is used here, starting with sugar alcohol base.
% Obtain stereochemically-agnostic thermo properties (Gf, Hf, V, Cp, and Gh) of 4, 5, or 6 carbon long sugar alcohols by regression of available data.
% Subtraction and addition of bits according to 2nd-order group additivity theory to obtain estimates of non-composite, non-amine, 6-carbon BHP headgroup properties.
% subtract 1,2-diol and add monoethanolamine to obtain amine forms.
% add properties of the monoethanolamine ionization reaction to estimate ionized amine BHP headgroups.
% HKF parameters for neutral BHP headgroups were obtained using the estimation scheme for polar neutral solutes reported in PS2001. HKF params for charged BHP headgroups were obtained using [check HKF calc].


\subsection{Estimation of revised Helgeson-Kirkham-Flowers (HKF) equation of state parameters for BHP polar headgroups}

Fitting parameters for the revised HKF equation of state were estimated for BHP polyol and aminopolyol headgroups using $\Delta_{f}G^{\circ}_{aq}$, $\Delta_{f}H^{\circ}_{aq}$, $Cp^{\circ}_{aq}$, $V^{\circ}_{aq}$, and $\Delta_{h}G^{\circ}_{aq}$ and the correlation strategies given in \cite{plyasunov2001correlation} for aqueous nonelectrolytes.

HKF parameters for ammoniumpolyol BHP headgroups were estimated using a hybrid estimation scheme:

shock and helgeson 1990 to get omega (est. from entropy), a2, a4, c1, c2 of charged molecule, get a1 from sverjenski 2014 (works for charged and uncharged), solve for a3.

\subsection{thermo calcs}
Used the same method for calculating relative percent abundances along Eh gradients as in ch2 and used same basis species (different Eh ranges, which are ...). Compared observed ratios of tetrafunctional and pentafunctional. Calc'd Eh at which thermodynamically stable ratio matched observation and called this headgroup-predicted Eh.



\section{Results}
\paragraph*{Average oxidation state of carbon in BHP headgroups, Z\textsubscript{C}}
% ZC of BHP headgroups does not correlate with temperature, dissolved O2, or Eh.

\paragraph*{Eh estimated from ratios of summed tetra- and pentafunctional BHP headgroups}
