...
Ongoing efforts are needed to elucidate the factors that ultimately control lipid distribution in the environment. Our level of understanding of how certain lipid structural adaptations contribute to the biophysical properties or function of the membrane ranges widely, and the role of some structures are completely unknown. Lipids obviously need to function for an organism, but their structures must also have an acceptable biosynthetic cost. Each lipid structure has an energetic cost associated with its synthesis and modification, which may potentially change across an environmental gradient. It seems plausible that lipid structures offering the best functional or fitness advantage for the lowest cost would be selected over the course of evolution.

% need to get thermodynamic properties of alkyl chains to estimate equilibrium abundances... but how to go about this? Methods exist for estimating properties of molecules if there is no thermodynamic data, assuming the structure is known... but the chemical structures of individual alkyl chains aren't exactly known using this LCMS method! Inspired the use of site-averaged free alkyl chains as representatives of observed distributions of IPL alkyl chains in samples (don't spend too long on this section, because safac and rationale is described in detail below)
...
Thermodynamic properties for sample-averaged free alkyl chains were estimated assuming the species were in the aqueous phase. The standard state convention for species in the aqueous phase assumed unit activity in a hypothetical 1 mol$\cdot$kg$^{-1}$ solution referenced to infinite dilution at any temperature and pressure. The standard state used for liquid water was one unit activity of pure H$_{2}$0 at any temperature and pressure. The standard state used for gaseous species was unit fugacity of the ideal gas at 1 bar and any temperature. Hydration properties were chosen to reflect the equilibrium process of transferring 1 mole of the species in the gas phase to the aqueous phase \citep{plyasunov2000thermodynamic}.

Aqueous partial molal thermodynamic Gibbs free energies and enthalpies of formation from the elements ($\Delta_{f}G_{aq}^{\circ}$ and $\Delta_{f}H_{aq}^{\circ}$, respectively), volumes ($V_{aq}^{\circ}$), heat capacities ($Cp_{aq}^{\circ}$), and Gibbs free energies of the hydration process ($\Delta_{h}G^{\circ}$) were estimated for sample-averaged free alkyl chains in two steps. The goal of the first step was to estimate the properties of a saturated, straight aliphatic chain of length of nC and terminating at one end with a hybrid functional group. This hybrid group was a combination of alcohol, carboxylic acid, alkane, and amide, with the fraction of each set by observed mole fractions of ether, ester, C-C, and amide-linked IPL alkyl chains, respectively. The properties of this fatty alkyl chain, $\Xi_{fatty}$, were estimated with the equation
...
% water chemistry results in table \ref{tab:eh_results}. Differences in Eh predicted by o2/h2o, nitrate/nitrite, ammonia/ammonium, sulfide/sulfate. System not in equilibrium, so potentials not the same. Interestingly, 

Chemical formulae, partial molal thermodynamic properties, and HKF equation of state parameters estimated for each sample-averaged alkyl chain are reported in Table \ref{tab:IPL_thermo}.

% When allowing e- to toggle, what happens to relative IPL abundances? (bison figure)

% Thermodynamically, what might explain why some lipids are never most abundant? (e.g. OF4's chain length or combo of properties place its peak between OF3 and OF5

% What happens with predicted Eh? Interesting things happen between BP3 and BP4. Might be worthwhile to start at BP1 and walk through each until BP6.
...
\end{tabular}%
  ...
  \begin{tablenotes}
    ...
    \tr{results_alkyleh} Eh predicted from sample-averaged free alkyl chains as described in the text.
